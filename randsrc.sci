function M = randsrc(n,m,alpha,seed)
	//
		...
		//By specifying a two-row matrix as the third argument, an n x m matrix is created.
		//The first row of the third argument is taken as the vector alphabet.
		//The second row is taken as the probability of the corresponding alphabet.
		//
		//M = randsrc(n,m,[alphabets;probabilities],seed)
		//The fourth argument may be used to specify the seed for random number generation.
		//
		//Special case
	...
	select(argn(2)) //Select the action according to number of input arguments.
		...
		case 3 then //The alphabet is also specified.
			...
			[nr, nc] = size(alpha)
			if nr>2 then
				error('randsrc: The matrix alpha may have at most two rows.')
			end
			if nr==2 then
				//2 rows case. The second row is probability. Check if >0 and sums to 1.
				if sum(alpha(2,:)) ~=1 | or(alpha(2,:)<0) then
					error('randsrc: Second row of alpha are probabilities. They have to be positive, and must sum to 1.')
				end
			end
			if nr==1 then
				//Single row case. Each alphabet with equal probability 1/(number of alphabets)
				prob = ones(1,nc)/nc
				alpha = [alpha;prob]
			end
		case 4 then //The seed is also specified.
			if n<ceil(n) | m<ceil(m) | n<1 | m<1 then
				//n<ceil(n) if and only if n is not an integer.
				//m<ceil(m) if and only if m is not an integer.
				error('randsrc: The number of rows n and columns m must be positive integers.')
	...
	//
		...
		cumulProb = zeros(alpha(2,:))
		for i = 2:length(alpha(2,:))
			cumulProb(i)=cumulProb(i-1)+alpha(2,i-1)
		end
	...
	M=matrix(M,n,m)
	//
	if argn(2) == 4 then //If seed altered, restore the old one.
		grand('setsd',oldseed)
	end
	//
endfunction
